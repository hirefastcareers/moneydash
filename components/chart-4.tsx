    ...
    return (
        <Card className="flex flex-col max-md:py-4!">
            ...
            <CardContent className="flex-1">
                <ChartContainer config={chartConfig} className="mx-auto -mt-2 aspect-square max-h-60 min-h-48">
                    <PieChart>
            ...
            <CardFooter className="relative -mt-26 flex-col gap-2 text-sm">
                <div className="grid w-full grid-cols-3 gap-2">
                    {chartData.map((item, index) => (
                        <div key={index} className="flex flex-col items-center justify-center text-center">
                            <div className="mb-1 flex items-center gap-1.5">
                                <div className="size-1.5 rounded-full" style={{ backgroundColor: item.fill }} />
                                <span className="max-w-15 truncate text-xs">{item.title}</span>
                            </div>
